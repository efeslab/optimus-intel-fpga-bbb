...
int get_debug_csr(CSR_MGR csrs, struct debug_csr *dbgcsr) {

    dbgcsr->read_cnt = csrs.readCSR(MMIO_CSR_READ_CNT);
    dbgcsr->rdrsp_cnt = csrs.readCSR(MMIO_CSR_RDRSP_CNT);
    dbgcsr->read_total = csrs.readCSR(MMIO_CSR_READ_TOTAL);
    dbgcsr->write_cnt = csrs.readCSR(MMIO_CSR_WRITE_CNT);
    dbgcsr->wrrsp_cnt = csrs.readCSR(MMIO_CSR_WRRSP_CNT);
    dbgcsr->write_total = csrs.readCSR(MMIO_CSR_WRITE_TOTAL);
    dbgcsr->properties = csrs.readCSR(MMIO_CSR_PROPERTIES);
    dbgcsr->clk_cnt = csrs.readCSR(MMIO_CSR_CLK_CNT);
    dbgcsr->state = csrs.readCSR(MMIO_CSR_STATE);
    dbgcsr->report_reccnt = csrs.readCSR(MMIO_CSR_REPORT_RECCNT);
